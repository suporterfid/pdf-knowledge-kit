...
def test_rls_prevents_cross_tenant_access():
    ...
    with conn.cursor() as cur:
        cur.execute("SELECT set_config('app.tenant_id', %s, false)", (str(org2_id),))
        cur.execute("SELECT path FROM test_documents WHERE path = %s", ("secret_doc.pdf",))
        docs = cur.fetchall()
        assert len(docs) == 0, "Should not be able to see org1's document when tenant_id is org2"
        
        cur.execute("SELECT content FROM test_chunks WHERE content LIKE %s", ("%secret%",))
        chunks = cur.fetchall()
        assert len(chunks) == 0, "Should not be able to see org1's chunks when tenant_id is org2"
