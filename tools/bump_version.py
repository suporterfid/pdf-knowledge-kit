...
def format_version(
    major: int, minor: int, patch: int, pre_type: str = "", pre_num: int = 0
) -> str:
...
Examples:
  ...
  %(prog)s patch --pre alpha  # Increment prerelease (1.1.0-alpha.1 -> 1.1.0-alpha.2)
    ...
    try:
        ...
        _echo(f"Current version: {current_version}")
        ...
        _echo(f"New version: {new_version}")

        if args.dry_run:
            _echo("\n[DRY RUN] Would update the following files:")
            _echo(f"  - {project_root / 'app' / '__version__.py'}")
            _echo(f"  - {project_root / 'frontend' / 'package.json'}")
            _echo(f"  - {project_root / 'package.json'}")
            return 0
        ...
        _echo("\nNext steps:")
        _echo("  1. Review changes: git diff")
        _echo("  2. Update CHANGELOG.md with release notes")
        _echo(f"  3. Commit: git commit -m 'chore: bump version to {new_version}'")
        _echo(f"  4. Tag: git tag -a v{new_version} -m 'Release version {new_version}'")
        _echo("  5. Push: git push && git push --tags")

        return 0

    except Exception as e:
        _echo(f"Error: {e}", stream=sys.stderr)
        return 1
