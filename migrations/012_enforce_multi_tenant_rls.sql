...
-- Migration: enforce consistent multi-tenant RLS policies across tenant scoped tables
-- Canonicalizes to tenant_id + app.current_tenant_id() with USING/WITH CHECK.
...
BEGIN
  ...
  LOOP
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I (tenant_id);', rec.index_name, rec.table_name);
